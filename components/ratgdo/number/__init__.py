...
from .. import CONF_RATGDO_ID, RATGDO_CLIENT_SCHMEA, ratgdo_ns, register_ratgdo_child

DEPENDENCIES = ["ratgdo"]

# Track which (door, number type) pairs have been used so each ratgdo door can
# have its own full set of numbers.
...
def validate_unique_type(config):
    """Validate that each number type is only used once per ratgdo door."""
    key = (str(config[CONF_RATGDO_ID]), config[CONF_TYPE])
    if key in USED_TYPES:
        raise cv.Invalid(
            f"Only one number of type '{config[CONF_TYPE]}' is allowed per ratgdo door"
        )
    USED_TYPES.add(key)
    return config
