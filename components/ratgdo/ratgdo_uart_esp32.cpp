...
static size_t next_uart_slot = 0;
    ...
    int tx_pin, bool invert)
{
    ...
    if (next_uart_slot >= UART_SLOT_COUNT) {
        ESP_LOGE(TAG,
            "No free hardware UART for ratgdo (max %u doors on this ESP32)",
            (unsigned) UART_SLOT_COUNT);
        return;
    }
    const RatgdoUartSlot& slot = UART_SLOTS[next_uart_slot++];
    this->uart_num_ = slot.port;
    this->tx_signal_idx_ = slot.tx_signal_idx;
...
void RatgdoUART::transmit_secplus2_preamble()
{
    ...
    esp_rom_gpio_connect_out_signal(this->tx_pin_, this->tx_signal_idx_, false, false);
    esp_rom_delay_us(SIGNAL_SETTLE_US);
}
