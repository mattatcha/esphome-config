...
def validate_single_light(config):
    """Validate that only one RATGDO light is configured per door."""
    light_id = str(config[CONF_RATGDO_ID])
    if light_id in USED_LIGHTS:
        raise cv.Invalid("Only one RATGDO light is allowed per ratgdo door")
