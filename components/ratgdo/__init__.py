...
# Multiple doors on one board: each instance is assigned its own hardware UART
# (see ratgdo_uart_esp32.cpp). ESP32 has two spare UARTs, so up to 2 doors.
MULTI_CONF = True
...
def FILTER_SOURCE_FILES() -> list[str]:
    ...
    # With MULTI_CONF, CORE.config[DOMAIN] is a list of instances rather than a
    # single dict. Keep any protocol source that is selected by at least one door.
    conf = CORE.config.get(DOMAIN, {})
    instances = conf if isinstance(conf, list) else [conf]
    selected = {inst.get(CONF_PROTOCOL, PROTOCOL_SECPLUSV2) for inst in instances}
    return [
        source
