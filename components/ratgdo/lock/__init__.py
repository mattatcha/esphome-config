...
def validate_single_lock(config):
    """Validate that only one RATGDO lock is configured per door."""
    lock_id = str(config[CONF_RATGDO_ID])
    if lock_id in USED_LOCKS:
        raise cv.Invalid("Only one RATGDO lock is allowed per ratgdo door")
